...
try {
    ...
    Interaction.prototype.defer = function (ephemeral) {
        return this.respond({
            ...
            .then(() => this.type === "MESSAGE_COMPONENT" ? this.message : this.originalResponse(ephemeral));
    };

    Interaction.prototype.updateMessage = function (content, options) {
        return this.respond({
            type: 7,
            data: parseMessageOptions(content, options, true)
        })
            .then(() => this.message);
    };

    Interaction.prototype.respond = function (data, fetch = false) {
        let parsed = Interaction.parseResponseData(data);
        let ephemeral = Boolean(parsed.data && (parsed.data.flags & FollowupMessage.Flags.EPHEMERAL));
        return phin({
            ...
            .then((res) => {
                if (res.statusCode >= 200 && res.statusCode < 300) {
                    if (fetch) {
                        return this.originalResponse(ephemeral);
                    }
                }
                else {
    ...
    Interaction.prototype.originalResponse = function (ephemeral = false) {
        if (ephemeral) {
            let response = new EphemeralResponse(this.client, this, this.channel);
            this.followups.cache.set(this.id, response);
            return response;
        }
        else {
            return phin({
